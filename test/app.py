...
def dashboard():
    ...
    daily_sales = [0] * last_day
    daily_costs = [0] * last_day

    sales = Sale.query.filter(Sale.date.between(f"{year}-{month:02d}-01", f"{year}-{month:02d}-{last_day:02d}")).all()
    costs = Cost.query.filter(Cost.date.between(f"{year}-{month:02d}-01", f"{year}-{month:02d}-{last_day:02d}")).all()

    for sale in sales:
        daily_sales[sale.date.day - 1] += sale.amount

    for cost in costs:
        daily_costs[cost.date.day - 1] += cost.amount
