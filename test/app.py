...
from models import db, User, Sale, Cost, RolePermissions, CostCategory, SafeTransaction, Product, Dish, RecipeItem, PriceHistory
...
def update_price(product_id):
    ...
    db.session.commit()
    flash('Cena zaktualizowana', 'success')
    return redirect(url_for('foodcost', tab='products'))
...
def api_get_price_history():
    ...
    if from_date:
        query = query.filter(PriceHistory.date >= datetime.strptime(from_date, '%Y-%m-%d').date())
    if to_date:
        query = query.filter(PriceHistory.date <= datetime.strptime(to_date, '%Y-%m-%d').date())
    records = query.order_by(PriceHistory.date).all()
    data = [{'date': ph.date.strftime('%Y-%m-%d'), 'price': ph.price_per_unit} for ph in records]
    return jsonify(data)

@app.route('/api/price_history', methods=['POST'])
def api_add_price_history():
