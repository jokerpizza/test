...
def api_sejf_saldo_summary():
    ...
    initial = SafeTransaction.query.filter(SafeTransaction.date < start).all()
    running = sum(t.amount if t.type=='wpłata' else -t.amount for t in initial)
    ...
    history = []
    for t in txs:
        amt = t.amount if t.type=='wpłata' else -t.amount
        running += amt
        history.append({'date': t.date, 'balance': running})
    # current balance at end = running
    deposits = sum(t.amount for t in txs if t.type=='wpłata')
    withdrawals = sum(t.amount for t in txs if t.type!='wpłata')
    # transactions list
    tx_list = [{'date':t.date, 'type':t.type, 'amount':t.amount, 'balanceAfter':h['balance'], 'user':t.user.username} for t,h in zip(txs, history)]
    return jsonify({'currentBalance': running, 'deposits': deposits, 'withdrawals': withdrawals, 'history': history, 'transactions': tx_list})
