...
from models import db, User, Sale, Cost, RolePermissions, CostCategory, SafeTransaction, Threshold
...
@app.route('/sejf_saldo')
@login_required
def sejf_saldo_page():
    return render_template('sejf_saldo.html')

@app.route('/api/sejf_saldo/summary')
@login_required
def api_sejf_saldo_summary():
    start = request.args.get('start')
    end = request.args.get('end')
    # Get transactions before range for initial balance
    initial_tx = SafeTransaction.query.filter(SafeTransaction.date < start).all()
    balance = sum(t.amount if t.type=='deposit' or t.type=='wpłata' else -t.amount for t in initial_tx)
    # In-range transactions
    txs = SafeTransaction.query.filter(SafeTransaction.date>=start, SafeTransaction.date<=end)           .order_by(SafeTransaction.date).all()
    history = []
    for t in txs:
        amt = t.amount if t.type=='deposit' or t.type=='wpłata' else -t.amount
        balance += amt
        history.append({'date': t.date, 'balance': balance})
    deposits = sum(t.amount for t in txs if t.type=='deposit' or t.type=='wpłata')
    withdrawals = sum(t.amount for t in txs if not (t.type=='deposit' or t.type=='wpłata'))
    tx_list = [{'date': t.date, 'type': t.type, 'amount': t.amount,
                'balanceAfter': history[i]['balance'], 'user': t.user.username}
               for i, t in enumerate(txs)]
    return jsonify({'currentBalance': balance, 'deposits': deposits,
                    'withdrawals': withdrawals, 'history': history,
...
def api_thresholds():
    if request.method == 'GET':
        thr = Threshold.query.all()
        return jsonify([{'id': t.id, 'type': t.key, 'value': t.value} for t in thr])
    data = request.get_json()
    key = data.get('type')
    value = data.get('value')
    t = Threshold(key=key, value=value)
    db.session.add(t)
    db.session.commit()
    return jsonify({'id': t.id, 'type': t.key, 'value': t.value}), 201
