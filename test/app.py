...
def add_cost():
    ...
    now = datetime.now()
    total_monthly_cost = Cost.query.filter(
        extract('month', Cost.date) == now.month,
        extract('year', Cost.date) == now.year
    ).with_entities(db.func.sum(Cost.amount)).scalar() or 0
...
def prepare_db():
    db.session.execute(
        text("ALTER TABLE IF EXISTS cost ADD COLUMN IF NOT EXISTS category VARCHAR(100);")
    )
    db.session.commit()
