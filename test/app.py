...
def api_costs():
    if request.method == 'GET':
        query = Cost.query.join(CostCategory, Cost.subcategory_id == CostCategory.id)                          .join(CostCategory, CostCategory.parent_id == CostCategory.parent_id, isouter=True)
        if request.args.get('from'):
            query = query.filter(Cost.date >= request.args.get('from'))
        if request.args.get('to'):
            query = query.filter(Cost.date <= request.args.get('to'))
        if request.args.get('category'):
            query = query.filter(CostCategory.parent.name == request.args.get('category'))
        if request.args.get('subcategory'):
            query = query.filter(CostCategory.name == request.args.get('subcategory'))

        results = []
        for c in query.order_by(Cost.date.desc()).all():
            results.append({
                'id': c.id,
                'date': c.date.strftime('%Y-%m-%d'),
                'category': c.subcategory.parent.name if c.subcategory and c.subcategory.parent else '',
    ...
    if request.method == 'POST':
        ...
        subcat = CostCategory.query.filter_by(name=data['subcategory']).first()
        new_cost = Cost(
