...
# Konfiguracja bazy:
#  - Jeśli używasz PostgreSQL na Renderze, to odczytuj z DATABASE_URL,
#    w przeciwnym wypadku SQLite lokalnie (np. /tmp/pizzeria.db).
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('DATABASE_URL', 'sqlite:///pizzeria.db')
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False

# Klucz sesji (w produkcji trzymaj w zmiennych środowiskowych!)
...
@login_required
def dashboard():
    # Pobranie parametrów month i year z GET
    today = date.today()
    current_year = today.year
    current_month = today.month
...
def settings():
    if request.method == 'POST':
        # Dodanie nowej kategorii do listy w pamięci
...
def sejf_saldo():
    """Wyświetla i obsługuje stan sejfu (gotówka), 
       ...
       pokazuje listę wszystkich transakcji gotówkowych."""
    if request.method == 'POST':
    ...
    # Zbieramy wszystkie operacje gotówkowe
    ...
    # Wpłaty/wypłaty sejfu
    for st in safe_transactions:
        username = st.user.username if st.user else "Nieznany"
        if st.type == "wypłata":
            delta = st.amount  # Sejf zyskuje
        else:
            delta = -st.amount # Sejf traci

        transactions.append({
            "date": st.date,
            "type": st.type,   # "wpłata" / "wypłata"
            "raw_amount": delta,
            "user": username,
        })

    # Sortuj rosnąco po dacie, by obliczyć "stan po transakcji"
    transactions.sort(key=lambda x: x["date"])

    running_balance = 0
    for t in transactions:
        running_balance += t["raw_amount"]
        t["balance_after"] = running_balance

    # Jeśli chcesz najnowsze na górze - odwróć listę
    transactions.reverse()

    # Aktualne saldo to running_balance (po wszystkich transakcjach)
