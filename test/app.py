...
def api_costs_summary():
    ...
    if cmp_start and cmp_end:
        prev = get_data(cmp_start, cmp_end)
    elif start_prev and end_prev:
        prev = get_data(start_prev, end_prev)
    else:
        delta = parse_date(end) - parse_date(start)
        ns = parse_date(start) - delta - timedelta(days=1)
        ne = parse_date(end) - timedelta(days=1)
        prev = get_data(ns.isoformat(), ne.isoformat())
