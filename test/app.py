...
def api_sejf_saldo_summary():
    ...
    for t in txs:
        if t.type == 'wpłata':
            balance += t.amount; deposits += t.amount
        else:
            balance -= t.amount; withdrawals += t.amount
        history.append({'date': t.date, 'balance': balance})
    tx_list = [{'date':t.date, 'type':t.type, 'amount':t.amount, 'balanceAfter':history[i]['balance'], 'user':t.user.username} for i,t in enumerate(txs)]
    return jsonify({'currentBalance': balance, 'deposits': deposits, 'withdrawals': withdrawals, 'history': history, 'transactions': tx_list})
...
def api_thresholds():
    if request.method == 'GET':
        ...
        t = Threshold(key=data.get('type'), value=data.get('value'))
        db.session.add(t)
        db.session.commit()
