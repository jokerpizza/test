...
# Endpoint do podsumowania kosztów
def parse_date(s):
...
def api_costs_summary():
    ...
    start_prev = request.args.get('start_prev')
    end_prev = request.args.get('end_prev')

    def get_data(s, e):
        s_date, e_date = parse_date(s), parse_date(e)
        costs = Cost.query.filter(Cost.date >= s, Cost.date <= e).all()
        total = sum(c.amount for c in costs)
        cats = {}
        for c in costs:
            cat = c.category.name
            sc = c.subcategory or 'Inne'
            if cat not in cats:
                cats[cat] = {'name': cat, 'amount': 0, 'sub': {}}
            cats[cat]['amount'] += c.amount
            cats[cat]['sub'].setdefault(sc, 0)
            cats[cat]['sub'][sc] += c.amount
        result = []
    ...
    if start_prev and end_prev:
        prev = get_data(start_prev, end_prev)
    else:
        delta = parse_date(end) - parse_date(start)
        new_start = parse_date(start) - delta - timedelta(days=1)
        new_end = parse_date(end) - timedelta(days=1)
        prev = get_data(new_start.isoformat(), new_end.isoformat())

    return jsonify({'total': data['total'], 'categories': data['categories'], 'prev': prev})
