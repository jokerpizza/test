...
def register_audit_listeners():
    def after_insert(mapper, connection, target):
        try: user_id = session.get('user_id') except: user_id = None
    ...
    def after_update(mapper, connection, target):
        try: user_id = session.get('user_id') except: user_id = None
        connection.execute(AuditLog.__table__.insert().values(
            user_id=user_id, action='update', object_type=mapper.class_.__name__, object_id=target.id, details=None
        ))
...
def logi():
    ...
    per_page = 50
    pagination = query.order_by(AuditLog.timestamp.desc()).paginate(page, per_page, error_out=False)
