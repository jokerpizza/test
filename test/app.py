...
import calendar
from flask import
import json Flask, render_template, request, redirect, url_for, session, flash, jsonify
...
def add_cost():
    if request.method=='POST':
        ...
        sub_cat = request.form['subcategory']
        desc = request.form['description']
        amt = float(request.form['amount'])
        pay = request.form['payment_method']
        new = Cost(date=date_val, category=f"{main_cat} / {sub_cat}", description=desc, amount=amt, payment_method=pay, user_id=session['user_id'])
        db.session.add(new)
        db.session.commit()
    ...
    return render_template('add_cost.html',
                           main_categories=[type('M',(object,),{'name':c.name,'icon':c.name.lower().replace(' ', '-')}) for c in main_categories],
...
'add_cost.html', main_categories=main_categories)
