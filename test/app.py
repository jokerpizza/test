...
@login_required
def costs_list():
    """Lista kosztów z możliwością filtrowania po query params:
    - date_from, date_to (YYYY-MM-DD)
    - category (substring)
    - cash / transfer checkboxes
    """
    q = Cost.query
    ...
    if category:
        q = q.filter(Cost.category.ilike(f'%{category}%'))
    cash = request.args.get('cash')
    transfer = request.args.get('transfer')
    # Jeśli nie zaznaczono żadnego checkboxa, akceptuj oba rodzaje płatności.
    if cash and not transfer:
        q = q.filter(Cost.payment_method == 'Gotówka')
    elif transfer and not cash:
        q = q.filter(Cost.payment_method == 'Przelew')
    costs = q.order_by(Cost.date.desc()).all()
    return render_template('costs_list.html', costs=costs, request=request)
