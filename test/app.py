...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://koszty_new_db_user:YFubyoOSJQCYAilFWMTiD9c74Cn9BJWv@dpg-d01c8eruibrs73akcthg-a.oregon-postgres.render.com/koszty_new_db"
    ...
    return jsonify([
        {
            ...
            'subcategory': c.category_rel.name if c.category_rel else c.category,
            'main_category': c.category_rel.parent.name if c.category_rel and c.category_rel.parent else None
        } for c in costs
    ])
...
def run_upgrade():
    ...
    upgrade()
    return "Migracja wykonana!"


@app.route('/admin/test-db-url')
def test_db_url():
    import os
    return f"DATABASE_URL = {os.environ.get('DATABASE_URL', 'NIE USTAWIONY')}"


@app.route('/admin/sqlalchemy-check')
def sqlalchemy_check():
    return f"SQLAlchemy is using URI: {app.config.get('SQLALCHEMY_DATABASE_URI', 'Not set')}"
