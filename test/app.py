...
@app.route('/api/safe/summary')
@login_required
def api_safe_summary():
    start = request.args.get('start')
    end = request.args.get('end')
    # Fetch transactions from Cost or SafeTransaction model
    ...
    current = balance
    # transactions list
    tx_list = [{'date':t.date,'type':t.type,'amount':t.amount,'balanceAfter': h['balance'],'desc':t.description,'user':t.user.username} for t,h in zip(txs, history)]
    return jsonify({'currentBalance':current,'deposits':deposits,'withdrawals':withdrawals,'history':history,'transactions':tx_list})
