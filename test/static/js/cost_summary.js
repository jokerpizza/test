...
window.addEventListener('DOMContentLoaded', () => {
  const {start,end} = getPeriod('month');
  document.getElementById('startDate').value = start;
  document.getElementById('endDate').value = end;
  document.getElementById('cmpStartDate').value = start;
  document.getElementById('cmpEndDate').value = end;
  const ctx1 = document.getElementById('pieChart').getContext('2d');
  pieChart = new Chart(ctx1, { type:'doughnut', data:{labels:[],datasets:[{data:[],backgroundColor:['#0d6efd','#198754','#ffc107','#dc3545','#6f42c1']}]}, options:{cutout:'60%',responsive:true} });
  const ctx2 = document.getElementById('compareChart').getContext('2d');
  compareChart = new Chart(ctx2, { type:'bar', data:{labels:[],datasets:[{label:'A',data:[],backgroundColor:'#0d6efd'},{label:'B',data:[],backgroundColor:'#6c757d'}]},options:{responsive:true,scales:{y:{beginAtZero:true}}} });
  updateAll(start,end,start,end);
});
