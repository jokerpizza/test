...
async function updateAll(start, end) {
  ...
  data.categories.forEach((c, i) => {
    const prevCat = prev.categories.find(p => p.name === c.name);
    const prevSum = prevCat ? prevCat.sub.reduce((sum,s)=> sum + s.amount, 0) : 0;
    const tr = document.createElement('tr');
    tr.innerHTML = `<td class="px-4 py-2">${c.name}</td><td class="px-4 py-2">${c.amount} zł</td><td class="px-4 py-2">${prevSum} zł</td>`;
